...
contract DeployMockToken is BaseScript {
    ...
    function _deployImplementation() internal override {
        ...
        for (uint256 i = 0; i < mockTokens.length; i++) {
            console2.log(
                "%s deployed at: %s",
                IERC20Metadata(address(mockTokens[i])).symbol(),
                address(mockTokens[i])
            );
        }

        string memory deployedMockTokens = string.concat(
            "\n# Deployed Mock Token contract addresses\n",
            "USDC=", vm.toString(address(mockTokens[0])), "\n",
            "WETH=", vm.toString(address(mockTokens[1])), "\n",
            "WBTC=", vm.toString(address(mockTokens[2])), "\n",
            "WSOL=", vm.toString(address(mockTokens[3])), "\n",
            "WLINK=", vm.toString(address(mockTokens[4])), "\n",
            "WAAVE=", vm.toString(address(mockTokens[5])), "\n"
        );
