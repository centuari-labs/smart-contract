...
library CentuariCLOBDSLib {
    ...
    string public constant ORDER_GROUP_ID_UINT256 = "ORDER_GROUP_ID";
    ...
    // Order Group data
    string public constant ORDER_GROUP_ID_INDEX_UINT256 = "ORDER_GROUP_ID_INDEX";
    string public constant ORDER_GROUP_AMOUNT_UINT256 = "ORDER_GROUP_AMOUNT";
    string public constant ORDER_GROUP_COLLATERAL_AMOUNT_UINT256 = "ORDER_GROUP_COLLATERAL_AMOUNT";
    string public constant ORDER_GROUP_STATUS_UINT256 = "ORDER_GROUP_STATUS";
    ...
    function getNextOrderGroupId(DataStore dataStore) internal returns (uint256 id) {
        bytes32 orderGroupIdKey = keccak256(abi.encodePacked(ORDER_GROUP_ID_INDEX_UINT256));
        id = dataStore.getUint(orderGroupIdKey) + 1;
        dataStore.setUint(orderGroupIdKey, id);
    }

    function getOrderGroupIdKey(uint256 orderGroupId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(ORDER_GROUP_ID_UINT256, orderGroupId));
    }

    function getOrderGroupAmountKey(uint256 orderGroupId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(ORDER_GROUP_AMOUNT_UINT256, orderGroupId));
    }

    function getOrderGroupCollateralAmountKey(uint256 orderGroupId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(ORDER_GROUP_COLLATERAL_AMOUNT_UINT256, orderGroupId));
    }

    function getOrderGroupStatusKey(uint256 orderGroupId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(ORDER_GROUP_STATUS_UINT256, orderGroupId));
    }
}
